...
- Web 页面提供任务、历史、搜索、进度、取消、重试、本地视频播放、文件操作和设置。
...
所有任务默认启用 HTTPS 证书兼容，包括携带 Cookie、Authorization 或标记为需要登录态的任务，以提高旧站点和自签名媒体服务的下载成功率。证书验证会独立执行并在任务详情中显示结果，不会关闭兼容模式或阻止下载。你仍可在 Web 新建下载、任务重试或插件的清晰度面板中为当前任务手动关闭兼容模式；该设置应用到清单、音轨、分片和关联媒体站点。
...
更新到包含数据库迁移的版本时，先运行 `pnpm build`，再执行 `pnpm stop` 和 `pnpm start`。服务启动时会自动迁移现有任务；随后在 Chrome 扩展管理页面重新加载插件。
...
任务卡片和详情页都可以删除任务。删除会停止仍在执行的下载、换封装或转码，并清理临时数据及内部播放副本；默认保留原始下载视频，只有在确认框中勾选“同时删除视频文件”才会删除原视频。

下载完成后，可以从任务卡片或详情页进入播放页。播放器按需读取本地文件并支持拖动进度，观看位置按播放文件身份保存在当前浏览器中。

新下载会自动检查格式：兼容的 MP4/WebM 直接播放；H.264 + AAC/MP3 位于 MKV、TS 等容器时，自动无损换封装为 MP4。其他编码会显示“生成兼容播放版本”，点击并确认后才开始有损转码。历史任务在首次进入播放页时检查，不会在升级后批量转码。

转码保持源分辨率和帧率，输出 H.264 + AAC MP4；奇数尺寸补齐为偶数，HDR 转为 BT.709 SDR，只保留第一路视频和音频。优先实际探测并使用 NVIDIA、Intel、AMD 硬件编码；不可用或失败时自动从头使用 CPU（libx264 CRF 21）。各硬件质量参数不保证与 CRF 21 完全相同。换封装和转码共用独立单任务队列，不占下载并发名额，但会使用磁盘、CPU 或显卡资源。

任务卡片、详情和播放页显示独立的播放准备状态，以及处理进度、速度和剩余时间；下载状态保持“已完成”。处理中可以取消、尝试原文件或使用系统播放器。取消会删除半成品；重启后未完成处理标记为中断，点击“重新准备”从头处理。兼容副本保存在 `%LOCALAPPDATA%/Ergou/playback/<任务 ID>/<文件指纹>/media.mp4`（可由 `ERGOU_DATA_DIR` 改变根目录），不覆盖原文件，删除任务时总是清理。
...
- 所有任务默认允许无效、过期或主机名不匹配的 HTTPS 证书。独立证书检测只显示结果，不影响下载；兼容模式不会绕过链接过期、登录校验、DRM 或媒体加密。
