...
任务卡片和详情页都可以删除任务。删除会停止仍在执行的下载并清理该任务的临时数据；默认保留已经下载的视频，只有在确认框中勾选“同时删除视频文件”才会删除成品。

下载完成后，可以从任务卡片或详情页进入播放页。播放器按需读取本地文件并支持拖动进度，观看位置保存在当前浏览器中。浏览器无法原生播放某种封装或编码时，可改用系统播放器打开；Ergou 不会为播放额外转码或生成副本。
