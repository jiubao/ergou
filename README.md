...
不携带浏览器登录信息的公开任务默认启用 HTTPS 证书兼容，以提高旧站点和自签名媒体服务的下载成功率。携带 Cookie、Authorization 或标记为需要登录态的任务默认保持证书校验。你可以在 Web 新建下载、任务重试或插件的清晰度面板中修改该选项；它只随当前任务保存，并应用到该任务的清单、音轨、分片和关联媒体站点。
...
- 公开任务默认允许无效、过期或主机名不匹配的 HTTPS 证书；含登录信息的任务默认验证证书。该选项不会绕过链接过期、登录校验、DRM 或媒体加密。
